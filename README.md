...
- **Navbar completeness**: added `aria-expanded`/`aria-controls` to the mobile menu toggle (was missing entirely), gave the mobile panel an `id` to match, and fixed the mobile "Start Free" button not closing the menu on click — plus a small `Button` component bug where `onClick` was silently dropped whenever `href` was also passed (the `Link` branch never forwarded it).
